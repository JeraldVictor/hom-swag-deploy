...
The `deploy` workspace provides a unified environment for running the entire HomSwag platform using Docker/Podman Compose. It manages the lifecycle of both the infrastructure services (MongoDB, Redis, MinIO) and the application services (Server, Admin, App).
...
- **Infrastructure**: Shared MongoDB, Redis, and MinIO instances managed as part of the compose stack.
- **Reverse Proxy**: Typically sits in front of the stack (e.g., Nginx) to handle TLS and routing to the specific ports (`3000`, `3001`, `3002`).
